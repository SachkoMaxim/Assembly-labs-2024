...
.data
	...
	SachkoHarpocrates_PaSswOrd db "ZeusOlympus",0
	...
	SachkoMinos_UseLEssWinDowHeADer db "You entered the Underworld (Lab 3)",0
	...
	SachkoEurydice_LeaVingMessenGEwiNDOWHeadER db "It was nice to know you. BYE",0
	...
	SachkoHecate_OpenAfterALLinfRMationHeaDEER db "Personal data of Maksym Sachko from the IM-22 group (option 20)",0
	...
	SachkoAlecto_passwordWrITewiNDowText db "Write the PAssWord to ENTER (not here)",0
...
.code
SachkoThirdLabUnderworld:
							
	SachkoRhadamanthus_UPvoteCorrECt proc
			invoke MessageBox, 0, addr SachkoMelinoe_trueOPenAFTERallInforMATIOnTexT, addr SachkoHecate_OpenAfterALLinfRMationHeaDEER, 0
	...
	SachkoPersephone_DownVotEincoRect proc
			invoke MessageBox, 0, addr SachkoGeras_wronGcouNTofSyMBols, addr SachkoOrm_wRongVeryWROngPAasSSworDHeADer, 0
			invoke ExitProcess, 0
		return 0
	...
	SachkoMinos_CHECKforCorReCTgoODPaSSword proc
		...
		mov eax, SachkoMegaera_nuMbeRofCHaraCTERSforMYSelf
		...
		mov bx, 0
		je SachkoJudge_MinosForCoRRecttHInG
		
		;invoke wsprintf, addr SachkoEmpusa_NUmbeRSStrinG8, addr SachkoSisyphus_nUmberNEed
		
		jmp SachkoPersephone_DownVotEincoRect
		
		invoke wsprintf, addr SachkoWhyNot_StringBuffer,
							addr SachkoWhyNot_StringBuffer,
							addr SachkoAthena_stRaTEgICgameFavoUriTe,
							addr SachkoApollo_NuMBerOFbook,
							addr SachkoHypnos_BirThDAtE
		...
		invoke wsprintf, addr SachkoCerberus_buffer, 
							addr SachkoCerberus_WronGVERYvroNGpasSwordText, 
							addr SachkoCerb_firstSentFORErrOrWiNDow, 
							addr Sachkoerus_SecOnDseNTfoRerRORwinDow
		
		SachkoAeacus_DownVotEincoRect:
			invoke MessageBox, 0, addr SachkoCerberus_truEWronGVERYvroNGpasSwordText, addr SachkoOrm_wRongVeryWROngPAasSSworDHeADer, 0
			invoke ExitProcess, 0
		return 0
		...
		SachkoJudge_MinosForCoRRecttHInG:
			.if bx != 12
				...
				mov dl, SachkoTisiphone_PAsswordKEapeR[bx]
			...
			;invoke wsprintf, addr SachkoEmpusa_NUmbeRSStrinGfive, addr SachkoSisyphus_nUmberNEed
	...
	SachkoHades_WinDoWhaNdleR proc hWnd:DWORD, userMsg:DWORD, wParam:DWORD, lParam:DWORD
		...
		.elseif userMsg == WM_INITDIALOG
			...
			;invoke wsprintf, addr SachkoEmpusa_NUmbeRSStrinG14, addr SachkoSisyphus_nUmberNEed
			invoke MessageBox, 0, addr SachkoThanatos_TRueFiRStWInDoWGreAtIng, addr SachkoMinos_UseLEssWinDowHeADer, 0
			;invoke wsprintf, addr SachkoEmpusa_NUmbeRSStrinG14, addr SachkoSisyphus_nUmberNEed
			...
			invoke MessageBox, 0, addr SachkoCharon_trueSecondDDWinDOwGREating, addr SachkoMinos_UseLEssWinDowHeADer, 0
		...
		.elseif userMsg == WM_COMMAND
			...
			.else
				.if wParam == 793
					...
					;invoke wsprintf, addr SachkoEmpusa_NUmbeRSStrinG14, addr SachkoSisyphus_nUmberNEed
					invoke MessageBox, 0, addr SachkoOrpheus_LeAvingMeSSeNgEWinDoWTexT, addr SachkoEurydice_LeaVingMessenGEwiNDOWHeadER, 0
					;invoke wsprintf, addr SachkoEmpusa_NUmbeRSStrinG14, addr SachkoSisyphus_nUmberNEed
				...
				.else
					.if wParam == 791
						...
						;invoke wsprintf, addr SachkoEmpusa_NUmbeRSStrinG14, addr SachkoSisyphus_nUmberNEed
						invoke MessageBox, 0, addr SachkoZagreus_jusTaWInDOwtExT, addr SachkoAlecto_passwordWrITewiNDowText, 0
						;invoke wsprintf, addr SachkoEmpusa_NUmbeRSStrinG14, addr SachkoSisyphus_nUmberNEed
	...
	SachkoTemp:
		Dialog "Write the PAssWord to ENTER", "Comic Sans MS", 9, WS_OVERLAPPED or WS_SYSMENU or DS_CENTER, 8, 216, 811, 240, 145, 1024
